...
class _AppendContentModulePageState extends State<AppendContentModulePage1>
  ...
  Future<void> _appendContent(String newContent) async {
  ...
  void _addTextContent() async {
    ...
    String sanitizedContent = contentController.text
        .replaceAll('"', '\\"')
        .replaceAll('\n', '\\n');
    String newContent = '{"$selectedType": "$sanitizedContent"}';
    await _appendContent(newContent);
  }

  void _addYouTubeContent() async {
    if (!_formKey.currentState!.validate()) return;
    if (contentController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('YouTube URL cannot be empty')),
      );
      return;
    }
    String newContent = '{"youtubevideo": "${contentController.text}"}';
    await _appendContent(newContent);
  }
    ...
    try {
      ...
      if (result != null && result.files.single.bytes != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Image selected: ${_mediaFile!.name}. Starting upload...',
            ),
          ),
        );
        await _uploadImage();
  ...
  Future<void> _uploadImage() async {
    if (_mediaFile == null || _mediaFile!.bytes == null) {
      ScaffoldMessenger.of(
        context,
    ...
    try {
      ...
      String newContent = '{"imageurl": "${response.secureUrl}"}';
      await _appendContent(newContent);
    } catch (e) {
      setState(() {
  ...
  Future<void> _handleVideoUpload() async {
    ...
    try {
      ...
      final videoId = await _uploadVideo(accessToken);
      if (videoId != null) {
        final videoUrl = 'https://www.youtube.com/watch?v=$videoId';
        String newContent = '{"youtubevideo": "$videoUrl"}';
        await _appendContent(newContent);
      } else {
        setState(() {
  ...
  Future<String?> _uploadVideo(String accessToken) async {
    try {
      ...
      int bytesSent = 0;
      final fileLength = _mediaFile!.bytes!.length;
      ...
      final stream = Stream.fromIterable([_mediaFile!.bytes!]);
      stream.listen(
        (data) {
        ...
        return Column(
          ...
          children:
              parsedContentList.expand((item) {
                String? url;
                if (item is Map) {
                  if (item.containsKey('imageurl')) url = item['imageurl'];
                  if (item.containsKey('youtubevideo'))
                    url = item['youtubevideo'];
                  if (item.containsKey('media'))
                    url = item['media']; // Backward compatibility
                }
                if (url != null) {
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 10),
                            currentContent.isNotEmpty && currentContent != '[]'
                          ...
                          child: Form(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                ] else if (selectedType == 'imageupload') ...[
                                  Column(
                                    children: [
                                      const Text(
                                        'Note: Supports PNG, JPEG, and other image formats.',
                                        style: TextStyle(
                                          fontStyle: FontStyle.italic,
                                          color: Colors.black54,
                                        ),
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    if (selectedType == 'youtubevideo')
                                      ElevatedButton(
                                        onPressed: _addYouTubeContent,
                                        style: ElevatedButton.styleFrom(
                                          backgroundColor: Colors.blueAccent,
                                          foregroundColor: Colors.white,
                                          shape: RoundedRectangleBorder(
